...
function supplyFood() {
	if(foodCount  < foodMax) {
		...
		var randomX = (Math.random() * 3980) + 10;
		var randomY = (Math.random() * 3980) + 10;
		supplyFoodOn(randomX, randomY, true);
	}
}

function supplyFoodOn(x, y, autoGenerated, points) {
	var food = new Food(foods.length, x, y, points);
	foods[food.id] = food;
...
io.sockets.on('connection', function(socket) {
	...
	function nitroLoop() {
		if(worms[myID].score >= 5) {
			worms[myID].score -= 5;
			supplyFoodOn(worms[myID].tail().x, worms[myID].tail().y, false, 5)
		} else {
			onSlowDown();
		}
	}
